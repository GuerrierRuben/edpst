  ...
  const handleShare = async (img) => {
    ...
    try {
      if (navigator.share) {
        ...
        alert("Lien de la galerie copie !");
      }
    } catch (err) {
              ...
              {images.map((img, i) => (
                ...
                >
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col justify-end p-4">
                    {img.title && (
                      <p className="text-white text-sm font-bold line-clamp-2 mb-2">{img.title}</p>
                    )}
                    {img.category && (
                      <span className="text-[10px] text-gray-300 font-medium px-2 py-1 bg-white/20 backdrop-blur-md rounded-full inline-block w-fit">
                        {img.category}
                      </span>
                    )}
                    <button 
                      ...
                      className="mt-3 flex items-center gap-2 bg-white/10 hover:bg-white/20 text-white px-3 py-1.5 rounded-lg text-xs font-bold transition-all w-fit"
                    >
          ...
          >
            ...
            <div className="absolute bottom-8 left-1/2 -translate-x-1/2 text-center">
              <p className="text-white text-lg font-bold mb-2">
                {images[selectedIndex]?.title || "Photo galerie"}
              </p>
              <button 
                onClick={(e) => {
                  e.stopPropagation();
                ...
                className="flex items-center gap-2 bg-white/10 hover:bg-white/20 text-white px-4 py-2 rounded-full text-sm font-bold transition-all mx-auto"
              >
